# 10. Define non-interactive and quiet CLI flags
...
We will introduce a `--non-interactive` flag that controls prompt policy. When `--non-interactive` is set, commands must not prompt. If any required input that does not have a default is missing, the command must exit with an error. Inputs with defined defaults may still use those defaults.
...
TTY detection only controls whether a renderer or interactive UI is used, not whether prompts are allowed. Non-TTY mode is treated as non-interactive by default, so it follows the same "no prompts and fail fast when required input is missing" rule.

Examples:
- `mmm --non-interactive init -l fabric -g 1.21.1 -m ./mods` runs without prompts and uses defaults for any optional values.
...
- `mmm --non-interactive --quiet test 1.20.4` runs without prompts and limits output to errors and required results.
...
Commands and shared helpers will need to be updated to follow this separation. Tests and documentation must be updated to reflect the new `--non-interactive` flag and the fact that `--quiet` no longer controls prompting.

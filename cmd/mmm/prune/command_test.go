...
func TestPruneOptionsFromFlagsReturnsLockSyncFlagError(t *testing.T) {
	cmd := &cobra.Command{}
	...
	_, err := pruneOptionsFromFlags(cmd)
	assert.Error(t, err)
}
...
func TestCommandSuccessDeletesFiles(t *testing.T) {
	...
	exists, existsErr := afero.Exists(fs, filepath.Join(meta.ModsFolderPath(cfg), "unmanaged.jar"))

...
## Behaviour
1. When unmanaged jar files are detected in the mods folder, print the unmanaged files notice defined in `docs/interactions/interaction-guidelines.md` and exit with code `1`.
2. Determine the version to test (argument or `latest`). If `latest` is used, the command queries the official API for the newest release.
3. For each mod in `modlist.json`, fetch metadata for the target version and loader.
4. If every mod has a valid file, the command prints a success message and exits with code `0`.
5. If any mod lacks support, the missing mods are listed and the command exits with code `1`.
6. Supplying the version currently in use results in a no-op with exit code `0`.

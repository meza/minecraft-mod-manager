...
## Behaviour
1. Determine the configuration file location. If it already exists and the `--non-interactive` flag is not used, the user is asked whether to overwrite or provide a new file name.
...
- When running in `--non-interactive` mode and the config file already exists, the command exits with an error.

## User Interaction
The user may be prompted to confirm overwriting files, choose values from lists or provide text input. Prompts are skipped when the relevant flags are supplied, or when running in non-interactive mode.

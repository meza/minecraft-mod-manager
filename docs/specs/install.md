...
2. When unmanaged jar files are detected in the mods folder, print the unmanaged files notice defined in `docs/interactions/interaction-guidelines.md` and exit without installing mods.
...
4. When lock entries are missing for configured mods, add them after download. Otherwise, leave existing lock entries intact.
5. When a mod author has changed the mod name, update the name in `modlist.json` and the lock file entry for that mod.

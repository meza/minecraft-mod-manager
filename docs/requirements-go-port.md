...
Global CLI flags include `--config` for selecting an alternative config file, `--unattended` to disable prompts and fail fast when required inputs are missing, `--quiet` to suppress non-essential output, and `--debug` for verbose output. The Node version loads environment variables via `dotenv`, allowing values to be stored in a local `.env` file.

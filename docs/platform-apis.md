...
Each hash triggers
`GET https://api.modrinth.com/v2/version_file/{hash}?algorithm=sha1`. Successful responses contain the mod version with file metadata, and failures are ignored. All successful results are collected into `PlatformLookupResult` objects.

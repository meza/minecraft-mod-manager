...
You see a check mark when a mod has a matching lock entry and the file exists; a cross means the lock entry is missing or the file cannot be found. When output is not colorized, the command uses V for installed mods and X for missing mods. If no mods are configured you will see a short notice instead.

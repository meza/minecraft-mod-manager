...
`mmm list` reports every mod config and the installation evidence MMM can establish without changing the modlist, lock data or files.

```bash
mmm list
```

The command has no command-specific options. Global options, missing setup and mod config correction are covered in [shared command behavior](README.md#modlist-and-setup).

## What the report distinguishes

For each mod config, the report distinguishes:
...
A matching filename alone does not prove that a mod is correctly installed. When the lockfile is missing, `list` reports the missing resolution evidence and does not create a lockfile or resolve new artifacts.

Malformed lock data, contradictory resolutions and entries without required integrity information are reported without trusting or rewriting that evidence. Conflicting duplicate mod configs are likewise reported rather than resolved by inspection. Identical duplicate mod configs are not silently deduplicated by `list`.
...
Visible unmanaged jars are reported after or alongside the managed results without preventing the modlist from being shown. Files matched by `.mmmignore` and files ending in `.disabled` are excluded from unmanaged reporting.

## Read-only guarantee and results

`list` does not repair files, reconcile lock entries, adopt jars, or rewrite the modlist. If interactive setup or a duplicate mod config correction is explicitly accepted, that shared flow is a separate change; `list` then resumes against the accepted setup or correction. Declining, cancelling or failing that flow leaves the original list operation unrun.

...
The applicable authorities are the command's document under `docs/interactions/flows`, its document under `docs/specs`, and the shared [interaction guidelines](../interactions/interaction-guidelines.md). Classification is `documented, implementation-coupled` unless stated otherwise. Fixtures were inline, in an afero memory filesystem, or in a test-owned temporary directory; there are no external fixture files.

...
- MMM retains provider-specific request construction, headers, parsing and selection. Its shared HTTP and download code retains retries, rate limiting, response validation and filesystem operations.
...
Keep fixture handlers alongside the E2E scenarios and reusable product actions. Do not build an expectation DSL, matcher registry, request planner or general mocking framework. Share concrete fixture data and handlers where scenarios need the same provider behavior.

## Planned endpoint configuration

The E2E harness will start one server per scenario on an automatically allocated loopback port, then pass all four values to each MMM process it launches. Distinct path prefixes can separate provider APIs, the Minecraft manifest and downloads on that server.
...
- Assert the product through terminal observations, exit status and resulting files. For downloads, check the expected bytes and relevant configuration or lockfile state.
...
- An intentionally configured error response is valid fixture behavior. A missing handler or fixture failure must not satisfy a scenario expecting a provider failure.
...
The first implementation should demonstrate initialization using the local manifest, a compatible mod download from each provider, and provider-not-found responses that leave configuration unchanged. Independently verify unexpected-request detection, missing required requests and concurrent handlers.

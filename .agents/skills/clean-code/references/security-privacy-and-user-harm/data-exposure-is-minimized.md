...
function subscriptionSummary(customer):
    record = customerStore.fetchAllFields(customer)
    return record
```
...
```text
function subscriptionSummary(customer):
    summary = customerStore.fetchFields(customer, ["plan", "renewalDate"])
    return { plan: summary.plan, renewalDate: summary.renewalDate }
```
